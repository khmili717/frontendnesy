...
class _CreateAppointmentDialogState extends ConsumerState<CreateAppointmentDialog> {
  ...
  void initState() {
    ...
    if (_selectedPatient != null) {
      _cinCnamController.text = _selectedPatient!.cinCnam?.toString() ?? '';
    }
  }
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  // Title
                  const Text(
                    'Saisir les détails du rendez-vous',
                    style: TextStyle(
      ...
      children: [
        ...
        Container(
          ...
          child: TextFormField(
            ...
            readOnly: widget.preSelectedPatient != null,
          ),
        ),
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // Debug logging for alien investigation
      debugPrint('[ALIEN_DEBUG] 🛸 Creating appointment request:');
      debugPrint('[ALIEN_DEBUG] Doctor ID: ${_selectedDoctor!.id}');
      debugPrint('[ALIEN_DEBUG] Patient ID: ${_selectedPatient!.id}');
      debugPrint('[ALIEN_DEBUG] Date: ${_formatDateForApi(_selectedDate!)}');
      debugPrint('[ALIEN_DEBUG] Time: ${_formatTimeForApi(_selectedTime!)}');
      debugPrint('[ALIEN_DEBUG] Status: $_selectedStatus -> ${_selectedStatus.toBackendString()}');
      debugPrint('[ALIEN_DEBUG] Backend JSON: ${request.toBackendJson()}');

      final stopwatch = Stopwatch()..start();
      await ref.read(appointmentsProvider.notifier).createAppointment(request);
      stopwatch.stop();
      
      debugPrint('[ALIEN_DEBUG] 🛸 Appointment creation took: ${stopwatch.elapsedMilliseconds}ms');
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Rendez-vous créé avec succès pour ${_selectedPatient!.prenPatient} ${_selectedPatient!.nomPatient}',
            ),
      ...
      debugPrint('[ALIEN_DEBUG] 🛸 ERROR: $e');
      setState(() {
        _isSubmitting = false;
  ...
  String _getErrorMessage(String error) {
    if (error.contains('400')) {
      ...
      return 'Erreur lors de la création du rendez-vous. Veuillez réessayer.';
    }
  }
